...
from project_utility import get_products_by_id, get_additions_by_id, get_path_parameter, validate_location, validate_payment_information, build_response, deserialize_dynamo_object, serialize_to_dynamo_object, to_coffee_type, to_milk_type, COFFEE_TYPES, MILK_TYPES, validate_price
...
def validate_order_item(item, index, known_products, known_additions):
    ...
    if 'coffeeType' in item:
        ...
        if coffee_type is None or coffee_type not in validated_product['allowedCoffeeTypes']:
            return False, f"Order item {index+1} has invalid coffee type ({coffee_type}). Valid value(s): {', '.join(validated_product['allowedCoffeeTypes'])}"
    ...
    if 'milkType' in item:
        if 'allowedMilkTypes' not in validated_product:
            return False, f"Order item {index+1} specified a milk type but product doest not support milk types"

        milk_type = to_milk_type(item['milkType'])
        if milk_type is None or milk_type not in validated_product['allowedMilkTypes']:
            return False, f"Order item {index+1} has invalid milk type ({milk_type}). Valid value(s): {', '.join(validated_product['allowedMilkTypes'])}"
        
        validated_order_item['milkType'] = milk_type
    elif validated_product['allowedMilkTypes'] is not None:
        return False, f"Order item {index+1} must specify a milk type. Valid value(s): {', '.join(validated_product['allowedMilkTypes'])}"

    if 'additions' in item:
        validated_additions = []
        allowed_addition_ids = validated_product['allowedAdditions'] if 'allowedAdditions' in validated_product else []

        for addition in item['additions']:
            if 'id' not in addition:
                return False, f"Order item {index+1} must specify addition IDs"
            elif addition['id'] not in known_additions:
                return False, f"Order item {index+1} uses unknown addition {addition['id']}"
            elif addition['id'] not in allowed_addition_ids:
                return False, f"Order item {index+1} uses {addition['id']} which is not allowed on that product"
            
            valid_addition = known_additions[addition['id']]
            if not valid_addition['enabled']:
                return False, f"Order item {index+1} uses disabled addition {addition['id']}"
            else:
...
def create_order(customer_id, order):
    ...
    if 'deliveryLocation' in order:
        is_valid, data = validate_location(order['deliveryLocation'], 'Delivery location')
        if is_valid:
    ...
    if 'payment' in order:
        is_valid, data = validate_payment_information(order['payment'], 'Payment information')
        if is_valid:
